...
Модели не закреплены: агенты наследуют модель сессии (`model: inherit`), Codex берёт модель из `~/.codex/config.toml`. Настройки харнесса — `plugin/config/defaults.json`, переопределение проекта — `.claude/claude-mini.json`, проверка — `plugin/bin/config validate`.
