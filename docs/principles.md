...
### 6. Project autonomy — команды per-project, не глобально

Slash-команды pipeline устанавливаются в `.claude/commands/` каждого проекта, а не в `~/.claude/commands/`. Глобальный `~/.claude/commands/` не содержит project-bound команд.

**Почему:** глобальный namespace создаёт невидимые зависимости между проектами. Изменение одной команды ломает все проекты одновременно. Ошибка-триггер: `/feature 90` переместил карточки на доске другого проекта, потому что `feature.md` был установлен глобально с захардкоженными ID.

**Материально:** `universal-setup.sh --target <repo>` копирует `bootstrap/commands/` в `<repo>/.claude/commands/` и записывает `<repo>/.claude/pipeline-version`. `--install` не трогает `commands/`. Каждый проект обновляется явно и намеренно. Drift виден в `/feature` startup. (ADR-0018)
