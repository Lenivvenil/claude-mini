...
1. **Issue type/priority triage — accept suggestions; defer automatic priority changes.**
   - **Current:** the issue prompt infers labels and restricts them to existing labels: [task-to-issue.md:20](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/commands/task-to-issue.md#L20), [:36](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/commands/task-to-issue.md#L36). Priority audit checks presence, not correctness: [ticket-audit.sh:86](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/scripts/ticket-audit.sh#L86).
...
2. **“Is an ADR needed?” — accept at planning; reject Jev as sole waiver.**
   - **Current:** v1 uses filename substrings: [governance-rules-lib.sh:25](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/hooks/governance-rules-lib.sh#L25), [:94](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/hooks/governance-rules-lib.sh#L94). Actual semantic triggers are in [adr-trigger.md:5](../../docs/runbooks/adr-trigger.md#L5). **The plugin already removed this path enforcement:** [commit-msg-check.sh:11](../../plugin/scripts/commit-msg-check.sh#L11).
...
3. **Which critics to invoke — accept additional recommendations; defer omission of required critics.**
   - **Current:** domain/prod-bound/docs paths and labels choose specialists; adversarial critic always runs: [feature.md:78](https://github.com/Lenivvenil/claude-mini/blob/2adf7a5/bootstrap/commands/feature.md#L78), [:86](https://github.com/Lenivvenil/claude-mini/blob/2adf7a5/bootstrap/commands/feature.md#L86). Current requirements remain in [dod-checklist.md:11](../../docs/runbooks/dod-checklist.md#L11).
...
4. **Severity/duplicate triage of critic and Codex findings — accept visible annotations and grouping.**
   - **Current:** reviewer severity rubric and preserved critic output: [review/SKILL.md:62](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/skills/review/SKILL.md#L62), [:80](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/skills/review/SKILL.md#L80). Codex findings require individual agree/disagree/unsure judgments: [codex-review/SKILL.md:29](../../plugin/skills/codex-review/SKILL.md#L29). Dedicated semantic deduplication is **not verified**.
...
5. **AC coverage / intent-check — accept evidence classification; reject proof of completion.**
   - **Current:** deterministic AC extraction followed by LLM `covered/partial/missing` classification and file evidence: [intent-check.md:42](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/commands/intent-check.md#L42), [:58](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/commands/intent-check.md#L58).
...
8. **Ticket audit — accept semantic testability checks; keep structural checks deterministic.**
   - **Current:** title/priority checks, a three-AC quota, and literal principle-reference checks: [ticket-audit.sh:77](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/scripts/ticket-audit.sh#L77), [:134](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/scripts/ticket-audit.sh#L134), [:154](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/scripts/ticket-audit.sh#L154).
...
9. **Commit type suggestion — technically suitable; defer for low incremental value.**
   - **Current:** deterministic allowed-type/format validation: [commit-msg-check.sh:62](../../plugin/scripts/commit-msg-check.sh#L62). Semantic type suggestion is **not verified**.
...
10. **Flaky versus real test failure — reject as a pass/fail decision; defer investigation routing.**
    - **Current:** test exit/timeout handling blocks on failure: [stop-hook.sh:225](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/hooks/stop-hook.sh#L225). A historical flakiness classifier/corpus is **not verified**.
...
11. **Handoff: “is there unfinished work?” — accept residual-obligation advice; reject Jev-owned completion.**
    - **Current:** mechanical handoff metadata/TODO warnings and preflight session/CI/issue/PR summaries: [stop-hook.sh:72](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/hooks/stop-hook.sh#L72), [:161](https://github.com/Lenivvenil/claude-mini/blob/62a2889/bootstrap/hooks/stop-hook.sh#L161), [mini-preflight.sh:125](../../bootstrap/scripts/mini-preflight.sh#L125).
...
12. **Eval grading of critics — accept as an assisted semantic grader.**
    - **Current:** an LLM PASS/FAIL causal rubric and separate deterministic invocation check: [finds-mismatch.md:2](../../plugin/evals/worktree-hook-mismatch/graders/finds-mismatch.md#L2), [used-critic.md:2](../../plugin/evals/worktree-hook-mismatch/graders/used-critic.md#L2).
...
- **One Python 3 CLI**, such as `plugin/scripts/jev.py`, using standard-library HTTPS/JSON and a small explicit response validator. Python is already required by [commit-msg-check.sh:47](../../plugin/scripts/commit-msg-check.sh#L47). Avoid another daemon, MCP server, runtime installer, or per-agent client.
...
Skills call the CLI at explicit intake/plan/review/handoff checkpoints and consume JSON. Agents use it for bounded classification, then retain reasoning responsibility. The existing hook has a verified **10-second timeout**: [hooks.json:13](../../plugin/hooks/hooks.json#L13).
