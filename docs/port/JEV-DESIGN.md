...
1. **Issue type/priority triage — accept suggestions; defer automatic priority changes.**
   - **Current:** the issue prompt infers labels and restricts them to existing labels: [task-to-issue.md:20](../../bootstrap/commands/task-to-issue.md#L20), [:36](../../bootstrap/commands/task-to-issue.md#L36). Priority audit checks presence, not correctness: [ticket-audit.sh:86](../../bootstrap/scripts/ticket-audit.sh#L86).
...
2. **“Is an ADR needed?” — accept at planning; reject Jev as sole waiver.**
   - **Current:** v1 uses filename substrings: [governance-rules-lib.sh:25](../../bootstrap/hooks/governance-rules-lib.sh#L25), [:94](../../bootstrap/hooks/governance-rules-lib.sh#L94). Actual semantic triggers are in [adr-trigger.md:5](../../docs/runbooks/adr-trigger.md#L5). **The plugin already removed this path enforcement:** [commit-msg-check.sh:11](../../plugin/scripts/commit-msg-check.sh#L11).
...
5. **AC coverage / intent-check — accept evidence classification; reject proof of completion.**
   - **Current:** deterministic AC extraction followed by LLM `covered/partial/missing` classification and file evidence: [intent-check.md:42](../../bootstrap/commands/intent-check.md#L42), [:58](../../bootstrap/commands/intent-check.md#L58).
...
6. **Plan quality lint — accept narrow semantic checks.**
   - **Current:** section/content checks plus per-line keyword/ADR-reference heuristics: [plan-lint.sh:59](../../bootstrap/scripts/plan-lint.sh#L59), [:83](../../bootstrap/scripts/plan-lint.sh#L83). This is not comprehensive plan-quality assessment.
...
7. **Hedging lint — accept contextual qualification; reject probabilistic word banning.**
   - **Current:** Semgrep execution and word/conditional rules: [hedging-lint.sh:34](../../bootstrap/scripts/hedging-lint.sh#L34), [hedging.yml:6](../../.semgrep/hedging.yml#L6).
...
8. **Ticket audit — accept semantic testability checks; keep structural checks deterministic.**
   - **Current:** title/priority checks, a three-AC quota, and literal principle-reference checks: [ticket-audit.sh:77](../../bootstrap/scripts/ticket-audit.sh#L77), [:134](../../bootstrap/scripts/ticket-audit.sh#L134), [:154](../../bootstrap/scripts/ticket-audit.sh#L154).
...
10. **Flaky versus real test failure — reject as a pass/fail decision; defer investigation routing.**
    - **Current:** test exit/timeout handling blocks on failure: [stop-hook.sh:225](../../bootstrap/hooks/stop-hook.sh#L225). A historical flakiness classifier/corpus is **not verified**.
...
11. **Handoff: “is there unfinished work?” — accept residual-obligation advice; reject Jev-owned completion.**
    - **Current:** mechanical handoff metadata/TODO warnings and preflight session/CI/issue/PR summaries: [stop-hook.sh:72](../../bootstrap/hooks/stop-hook.sh#L72), [:161](../../bootstrap/hooks/stop-hook.sh#L161), [mini-preflight.sh:125](../../bootstrap/scripts/mini-preflight.sh#L125).
