---
name: qa
description: QA gate — test coverage and docs currency check. Runs after /implement, before /review. Reads plan.md and writes qa-report.md. Use when running QA on a feature branch, after implementing a change, or when asked to "run qa", "check coverage", or "qa gate". Requires plan.md to be present.
---
...
- "run qa", "qa gate", "check coverage"
- After `/implement`, before `/review`
- "проверь тесты", "запусти qa"
- When `plan.md` and recent changes are present on the current branch

## Prerequisites

- `plan.md` present in repo root (or recent commit to check)
- `git` available with working tree or commit history
...
2. **Prompt-artifact** (evaluate before docs-only — bootstrap `.md` files are not docs): every changed file matches `plugin/agents/*.md`, `bootstrap/commands/*.md`, `bootstrap/skills/*/SKILL.md`, or `plugin/skills/*/SKILL.md`. If true → run `./scripts/lint-prompts.sh` against the matched files. Write `qa-report.md` with the linter output and verdict (PASS or FAIL). **Exit.** Do not proceed to step 3 or Phase 1. Mixed PRs (prompt artifact + other files) fall through to Phase 1 — manually invoke `./scripts/lint-prompts.sh` on the changed prompt-artifact files as part of Phase 2, then continue with the standard test-coverage check for any logic files.
...
2. Search for a test file using the bundled script:
   ```bash
   bash ~/.claude/skills/qa/scripts/find-tests.sh "<stem>"
   ```
   Falls back to inline `find` if script unavailable: `find . -name "test_${stem}*" -o -name "${stem}_test*" -o -name "*.bats" 2>/dev/null`.
...
`qa-report.md` written to repo root with:
...
Contents printed to conversation. Operator copies the `## QA` section into the PR body at `gh pr create` time.
