...
2. **Prompt-artifact** (evaluate before docs-only — bootstrap `.md` files are not docs): every changed file matches `plugin/agents/*.md`, `bootstrap/commands/*.md`, or `bootstrap/skills/*/SKILL.md`. If true → run `./scripts/lint-prompts.sh` against the matched files. Write `qa-report.md` with the linter output and verdict (PASS or FAIL). **Exit.** Do not proceed to step 3 or Phase 1. Mixed PRs (prompt artifact + other files) fall through to Phase 1 — manually invoke `./scripts/lint-prompts.sh` on the changed prompt-artifact files as part of Phase 2, then continue with the standard test-coverage check for any logic files.
